...
def delete_glossary():
    ...
    glossary_name = f"projects/{PROJECT_ID}/locations/{CATALOG_LOCATION}/glossaries/{GLOSSARY_ID}"
    try:
...
def delete_profile_scans():
    ...
    for tier in PROFILED_TIERS:
        for view_def in CORPUS:
            ...
            try:
                client.delete_data_scan(request=DeleteDataScanRequest(name=scan_name))
