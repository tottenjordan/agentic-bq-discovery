...
GUIDELINES_ASPECT_TYPE = "projects/dataplex-types/locations/global/aspectTypes/guidelines"
...
GLOSSARY_TERMS = {
    ...
    "fare-and-tip": {
        ...
        "description": (
            "Monetary amounts on a taxi trip: metered fare plus the gratuity paid by the passenger."
        ),
...
def create_and_run_profile_scans():
    ...
    for i, (tier, view_def) in enumerate(targets):
        ...
        try:
            response = client.run_data_scan(request=RunDataScanRequest(name=scan_name))
    ...
    if jobs:
        ...
        for scan_id, job_name in jobs:
            for _ in range(30):  # up to 5 minutes per scan
                time.sleep(10)
                job = client.get_data_scan_job(request=GetDataScanJobRequest(name=job_name))
        ...
        op = glossary_client.create_glossary(
            request=dataplex_v1.CreateGlossaryRequest(
                ...
                glossary=dataplex_v1.Glossary(
                    ...
                    description=("Business terms for the bigquery-context demo corpus."),
                ),
            )
        )
        op.result()
...
def main():
    ...
    print(
        f"  Corpus: {len(CORPUS)} tables × {len(TIERS)} tiers = {len(CORPUS) * len(TIERS)} views\n"
    )
